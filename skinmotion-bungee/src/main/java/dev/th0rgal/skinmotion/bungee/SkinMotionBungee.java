package dev.th0rgal.skinmotion.bungee;

import dev.th0rgal.skinmotion.bungee.commands.CapeCommand;
...
/**
 * Main entry point for the SkinMotion BungeeCord plugin.
 */
public final class SkinMotionBungee extends Plugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("SkinMotion (BungeeCord) enabled! API: " + config.getApiUrl());

        // Check API health
        getProxy().getScheduler().runAsync(this, () -> {
            if (apiClient.isHealthy()) {
                getLogger().info("Skin API is reachable and healthy.");
            } else {
                getLogger().warning("Skin API is not reachable at " + config.getApiUrl());
            }
        });
    }
    ...
    public void onDisable() {
        ...
        getLogger().info("SkinMotion (BungeeCord) disabled.");
    }
